...
namespace MeatRobot.Utils {
 ...
 // #if SOUND_MANAGER
  ...
  public void SetHoverBehaviourChangeColor(Color colorOver, Color colorOut) {
    ...
    void OnMouseDown() {
      if (!clickThroughUI && IsPointOverUI()) return; // Over UI!
     ...
     public void Manual_OnMouseExit() {
      ...
      void OnMouseEnter() {
        if (!clickThroughUI && IsPointerOverUI()) return;
       ...
       void OnMouseExit() {
        ...
        if (hoverBehavuour_Move) transform.localPosition = posExit;
  ...
  void Awake() {
    if (GetWorldCamera = null) SetGetWorldCamera(() => Camera.main);
